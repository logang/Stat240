...
%           CoeffGARCH     -- a matrix of coefficients from GARCH(1,1) fit
%                            - ith row = ith asset
%                            - 1st column = conditional mean
%                            - 2nd column = omega
%                            - 3rd column = beta
%                            - 4th column = alpha
%           CoeffAR        -- a matrix of coefficients from AR(1) fit
%                            - ith column = ith asset
%                            - 1st row: intercept
%                            - 2nd row: phi

function [PredMean, PredCov, Error, CoeffGARCH, CoeffAR, Innos] = Fit_AR_GARCH(returns, m)
    ...
    % AR(1) model
    % ---------------------
    CoeffAR = zeros(2,n); % initializes matrix of regression parameters
    ...
    for i = 1:n
        ...
        CoeffAR(:,i) = reg_mat\X(:,i); % solves for coeffs mu and phi
        X_fit(:,i) = reg_mat*CoeffAR(:,i); % fitted returns
    ...
    CoeffGARCH = zeros(n,4); % initializes coefficient matrix (one row per asset)
    ...
    PredMean = (CoeffAR(1,:))' +  diag(returns(m-1,:))*(CoeffAR(2,:)');
